        ...
        <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            <div class="inline-bloc min-w-full overflow-auto align-middle border-b border-gray-200 shadow sm:rounded-lg">
                    ...
                    <tbody class="bg-white">
                        ...
                        @forelse ($orders as $order)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <div class="text-sm leading-5 text-gray-500">{{ $order->token->name }}</div>
                                </td>

                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <div class="text-sm leading-5 text-gray-500">{{ $order->plan?->name }}</div>
                                </td>

                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <div class="text-sm leading-5 text-gray-500">{{ number_format($order->amount) }} {{$order->token->name}}
                                    </div>
                                </td>

                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <div class="text-sm leading-5 text-gray-500">{{ $order->percentage }}%</div>
                                </td>
                                <td
                                    class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">
                                    <button type="button" wire:click="processmodal({{ $order->id }})"
                                        class=" px-4 py-2 bg-indigo-500 text-white rounded-sb">Process</button>
                                </td>
                                <td
                                    class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">
                                    <button type="button" wire:click="deletemodal({{ $order->id }})">
                ...
                <x-confirmation-modal wire:model='deletingmodal'>
                    ...
                    <x-slot name="content">
                        {{ __('Are you sure you want to delete this deposit?') }}
                    </x-slot>
                ...
                <x-confirmation-modal wire:model='processingmodal'>
                    ...
                    <x-slot name="content">
                        {{ __('Are you sure you want to process this deposit?') }}
                    </x-slot>

                    <x-slot name="footer">
                        <x-button wire:click="$set('processingmodal', false)" class="ml-2"
